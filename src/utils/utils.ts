...
const logging = () => {
  ...
  ✅ Дополнительный функционал на выбор + 20
      - сохранение состояния страницы, изображение ёлки с размещёнными на ней игрушками добавляется в секцию "Вы нарядили". При клике по карточкам из этой секции восстанавливается состояние страницы с ёлкой на момент сохранения + 10

      Доп. функционал, не указанный в задании: + 10
      - удаление сохраненных елок (кнопка корзины на карточках елок)
      ...
      - при уменьшении страницы (меньше 860px) можно скрыть сайд-меню
  `)
}
